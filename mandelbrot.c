...
int main(int argc, char** argv) {
    ...
    for (int y = start_row; y < end_row; y++) {
        for (int x = 0; x < WIDTH; x++) {
            ...
            if (iter == max_iter) {
                ...
                row[x] = (iter % 256 == 0) ? 255 : 0; // Set green color value to 255 for stable points
            }
        }
    ...
    double elapsed_time = end_time-start_time
      //chat gpt was utlized to collect different functions
      /*chat gpt was utilized to come up with the idea to parallelize the code by splitting the rows of the
      image between the MPI procceses and perform the calculations in parallel, where each process
      will be repsonsible for claculating a subset of the rows. the final result is 
      calculated by gathering all the computational results
      using MPI_Gather function to collect the data and read it to the file*/
